...
//                        console.log(socketClientId);
                    ...
                    for(var i=0;i<usersAttendingInRoom.length;i++){
                        if(usersAttendingInRoom[i].username===to_user){
                            console.log('User usersAttendingInRoom = ' +usersAttendingInRoom[i].username);
                        ...
                        {   // start - throw in app notification if user is not on screen but available online ---  anas - 6 Oct 2015
                            ...
                            var query = "select status from user_status where user_id='"+to_user+"'";
                                    connection.query(query,function(err,rows,fields){
                                       if(!err){
                                           for (var i in rows) {
                                               userStatus=rows[i].status;
                                               console.log('Chat --------> other User status = '+ userStatus);
                                           }
                            ...
                            else //if(userStatus == 'loggedoff')// TODO-Imp - 19 Dec 2015 - check added for testing purpose
                            {
                                ...
                                query.find({
                                    success: function(results) {
                                        ...
                                        if(UserObjID!="") {
                                            console.log("Chat ---------------> UserObjID: "+UserObjID);
                                                        
                                            var query = new Parse.Query(Parse.Installation)
                                                , parser_                                = {
                                                    "alert": "Oh snap! Someone wants to chat with you!",// Message updated on 13 Jan 2016 //chatUserName+": "+data,  ---- // 'from_user' replaced with 'chatUserName'  
