    ...
    io.sockets.on('connection', function (socket) {
        ...
        var getUserStatus = function(response_to)
        {
            ...
            var query = "select status from user_status where user_id='"+response_to+"'";
                    ...
                    {
                        if (mainSocket) {
                            mainSocket.emit('message_user_isBusy', 'Event : message_user_isBusy -------------- Message : Sorry user has gone offline or busy in playing game with other user. Would you like to play with your preferences?');
                        }
                        // Treating second user as main user if requestor user is busy in playing or gone offline 
                        play(response_by, response_by_user_long, response_by_user_lat,'0');
                    }
                            ...
                            if(isAPNSResponsePlayRequest === false)
                            {
                                ...
                                //End
                                    ...
                                    for(var i in rows){
                                        ...
                                        if (rows[i].status == status_loggedIn || rows[i].status == status_play) { // TODO remove rows[i].status == status_loggedIn || //Fill only online user list found in preferences - 9 Sep 2015 - anas
                                               if(count<=5) {
                                        ...
                                        {
                                                if (mainSocket) {
                                                    mainSocket.emit('message_online_user', 'Sorry, User are online but not interested in play.');//, 'dwindle-any-user'
                                                }
                                        }
                                        else
                                        {       
        ...
        //                        console.log(socketClientId);
                ...
                if(isToUserInPlayScreen == false)
                {
                    ...
                    console.log('Play Screen - mainSocket = '+ mainSocket.username+' to_username = ' + toUserName);// comment this line
                            if (mainSocket) {
                                mainSocket.emit('message_game_started', 'Game Started',fromUser,'PlayScreen');
                            }
                            else 
                            {
                                console.log('send push notification if user press back.');
                                var fullDateTime = getCalculatedDateTime();//get current datetime - 14 Sep 2015 
