...
//var save_directory = "/home/ubuntu/dwindle_dating/public/uploadedImages/"; // TODO - uncomment this before deploying on server
//var pic_path1 = "http://159.203.245.103:3000/";// TODO - uncomment this before deploying on server
var save_directory = "public/uploadedImages/";// TODO - comment this before deploying on server
var pic_path1 = "http://localhost:3000/";// TODO - comment this before deploying on server
var io = null;
...
//                var user_Pics_Name = new Array();
                connection.query(query, function (err, rows, fields) {
                    if (!err) {
                        ...
                        var room = {name: roomname, userCount: 6, chatCount: 0, status: false, pic_count: 0, users: []};
                        rooms.push(room);
        ...
        socket.on('sendchat', function (data) {
            ...
            if (room.userCount > 2) {
                funGetUserFromDB(1, room.name, function (chatUser2) {
                    ...
                    //console.log("chatUser2: "+chatUser2);
                    secondChatUser = chatUser2;

                    //console.log('RoomChatCount: ' + room.chatCount + ", userChatCount: " + user.chatCount + ", MainUserChatCount: " + mainUser.chatCount);
					console.log(user.name +", -- "+secondChatUser);
                    //if (user.name === secondChatUser) {
...
//                                            io.sockets.in(socket.room).emit('adduserspic', JSON.stringify(userPicInfo));
                ...
                insertChatLogToDB(1, fromUser, toUser, fullDate,fullTime ,data, function (getResult) {
...
//            }
        ...
        function insertChatLogToDB(count, fromUser, toUser, fullDate,fullTime ,data, cb) {
            if(connection===null){
                connection=sqlConnection.handleDisconnect();
            }
            var strSql = "insert into chat_log(from_userid,to_userid,text,date,time) values('" + fromUser + "','" + toUser + "','" + data + "','" + fullDate +" " +fullTime+"','"+fullTime+"')";
