...
var play = function (io) {
    ...
    var secUserJson;
    var game_request_count = 0; // 22 Dec 2015
//    var sockets =[];
...
//     io.of('/').on('connection', function (socket) {
                                    ...
                                    success: function(results) {
                                        for(var a=0;a<results.length;a++){
                                          ...
                                          {*/
                                            ...
                                            /*else if(response_by===results[a].attributes.username){
                                        ...
                                        if(UserObjID!="") {
                                            ...
                                            var query = new Parse.Query(Parse.Installation)
                                                , parser_                                = {
                                                    "alert": response_by+": is waiting for to connect.",//+data,
    ...
    //                                    console.log("arguments", arguments);
        ...
        socket.on('Play', function (username, usrLon, usrLat, _page) {
            game_request_count = 0; // 22 Dec 2015
            play(username, usrLon, usrLat,_page);
        });//End Of Socket.on Play event

        //Start - Game Request Response Yes - 22 Dec 2015 
        ...
        //Start - Game Request Response No - 22 Dec 2015 
        socket.on('game_request_response_no', function (username, usrLon, usrLat,request_count) {//secondUser,secUser_Name,firstUser,user_Lat,user_Lon
            play(username, usrLon, usrLat,request_count);                                       
        });//End 
            ...
            {
                firstUserBoolean=true;// TODO-Imp- 21 Dec 2015
                ...
                var sql = "update user_status set status = 'play',location = POINT(" + usrLon + "," + usrLat + ") where user_id='" + username + "'";//, isPlaying = 1
                connection.query(sql, function (sqlerr, result) {
                    if (sqlerr) {
                                                ...
                                                for(var index =0 ; index < offline_user_rowsLength;  index++ )
                                                {
                                                    ...
                                                    for(var a=0;a<results.length;a++){
                                                     ...
                                                     if(UserObjID!="") {
                                                        ...
                                                        var query = new Parse.Query(Parse.Installation)
                                                            , parser_data = {
                                                                "alert": mainUserName+" is waiting for to connect.",//'from_user+": "+ removed on 22 dec 2015 ---- '+data 
                                ...
                                //End
                                    ...
                                    for(var i in rows){
                                        ...
                                        {       
                                                ...
                                                {
                                                    if (mainSocket) {
                                                        mainSocket.emit('message_user_found', user_list[0]);
                                                    }
                                                ...
                                                console.log("Find socket of second user - userSocket = "+ userSocket); //Comment this line  
                                                if (userSocket) {
                                                    game_request_count = game_request_count + 1;  // 22 Dec 2015
                                                    console.log("Play ------------> Second user is Connected with socket --- game_request_count = "+game_request_count);
                                                    //Start - 22 Dec 2015 - Ask second user is he/sher interested in play or not...
                                                    userSocket.emit('game_request', rows[i].name + ' wants to play with you. Are you interested in play?',rows[i].user_id,rows[i].name, user_name,user_Lat,user_Lon,game_request_count);//secondUser,secUser_Name,firstUser,user_Lat,user_Lon,game_request_count
                                                    //End
...
//                        console.log(strSql);
        ...
        socket.on('restartGamePlay', function () {
            ...
            // 'isPlaying = 1' added on 19 Dec 2015
            console.log('RestartGamePlay called ---->  with socket.username =' + socket.username);
            var sql = "update user_status set status = 'play' where user_id='" + socket.username + "'";//,isPlaying = 1 
            connection.query(sql, function (sqlerr, result) {
                if (!sqlerr) {
                ...
                var mainSocket = findSocket(toUserName, io.sockets.connected);
                        if (mainSocket) {
                          console.log('Play Screen - mainSocket = '+ mainSocket.username+' to_username = ' + toUserName);// comment this line
                            ...
                            mainSocket.emit('message_from_play_screen', 'Event : message_from_play_screen           Message : ' + data, fromUser, 'PlayScreen', mainUserName, 'playing');// TODO-Imp =  remove 'Event : message_from_play_screen           Message : ' +
                        };
            }//end - anas - 8 Oct 2015
