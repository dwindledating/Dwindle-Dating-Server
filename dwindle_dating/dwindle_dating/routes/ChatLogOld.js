...
//                  chat_array['RoomName']=from_user+"-"+to_user;
                  ...
                  funGetToUserPics(to_username,function(pic_json){
					  ...
					  io.of("/Chat").to(socket.id).emit('getChatLog',JSON.stringify(chatJSON));
...
//            console.log(fullDate);
          insertChatLogToDB(1, from_user, to_user, fullDate,fullTime ,data, msgStatus,function (getResult) {
...
var insertChatLogToDB=function(count, fromUser, toUser, fullDate,fullTime ,data,msg_Status,cb) {
    if(connection===null){
        connection=sqlConnection.handleDisconnect();
    }
...
var findSocket = function(socketId, sockets){
    for(var socketKey in sockets){
        ...
        if(socketObj.username === socketId){
            console.log(socketObj.username);
            return socketObj;
        }
    }
