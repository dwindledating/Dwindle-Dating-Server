...
//     io.of('/').on('connection', function (socket) {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('event_change_isPlaying_flag', function(user_fb_id){
            updateIsPlayingFlag(user_fb_id, 0);
        });
    ...
    //                                    console.log("arguments", arguments);
        ...
        socket.on('Play', function (username, usrLon, usrLat, _page) {
            ...
            var play = function (username, usrLon, usrLat,_page) 
            {
                ...
                var sql = "update user_status set status = 'play',location = POINT(" + usrLon + "," + usrLat + "), isPlaying = 1 where user_id='" + username + "'";
                connection.query(sql, function (sqlerr, result) {
                    if (sqlerr) {
        ...
        //                        console.log(socketClientId);
                ...
                if(isToUserInPlayScreen == false)
                {
                    ...
                    //console.log('Play Screen - mainSocket = '+ mainSocket.username+' to_username = ' + toUserName);// comment this line
                            if (mainSocket) {
                                console.log('Event message_game_started -----> Second User JSON : '+ socket.secondUserdata);// TODO-Imp - 19 Dec 2015
                ...
                else if(isSecondUserInPlayScreen == false)
                {
                    ...
                    console.log('Secod User -----> secondUser_fbId = '+ toUserName+ ' name = '+ user2.name);
                    var mainSocket = findSocket(toUserName, io.sockets.connected);
                            if (mainSocket) {
                              console.log('Event message_game_started -----> Second User JSON : '+ socket.secondUserdata);// TODO-Imp - 19 Dec 2015
                            ...
                            {
                                /*socket.username = secondUser_fbId;
                                console.log('Second User -----> toUserName = '+toUserName);
                                var secondUserSocket = findSocket(toUserName, io.sockets.connected);
                                if (secondUserSocket) {
                                    secondUserSocket.emit('message_game_started', 'Event : message_game_started ----------- Message : Game Started for second user',fromUser,'PlayScreen'); // TODO-Imp
                                }
                                else
                                {*/
...
//                        console.log(strSql);
        ...
        socket.on('restartGamePlay', function () {
            ...
            var sql = "update user_status set status = 'play',isPlaying = 1  where user_id='" + socket.username + "'";
            connection.query(sql, function (sqlerr, result) {
                if (!sqlerr) {
        ...
        socket.on('leaveGame', function () { // 19 dec 2015 - 'loggedout' rename with 'leaveGame'
            updateIsPlayingFlag(socket.username, 0);// 19 dec 2015 - anas
