    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('connect with socket', function(username){
            ...
            if(connection===null){
                ...
                var sql = "update user_status set status = 'play' where user_id='" + username + "'";// play replace with loggedin
                connection.query(sql, function (sqlerr, result) {
                    if (sqlerr) {
    ...
    //                                    console.log("arguments", arguments);
        ...
        socket.on('Play', function (username, usrLon, usrLat, _page) {
            ...
            var play = function (username, usrLon, usrLat,_page) 
            {
                ...
                socket.username = username; //TODO-Imp - Commented on 15 Dec 2015 For socket disconnect issue whiel screen switching
...
//                console.log(socket.username+" connected to "+roomname);
        ...
        socket.on('sendchat', function (data) {
            ...
            //start - TODO-Imp - anas - 15 Dec 2015
            var roomChatCount = 0;
            /*if(room.chatCount) 
            {
              roomChatCount = room.chatCount;
              console.log('Dwindle Down Count if(room.chatCount) : '+room.chatCount); // TODO uncomment this line
            }*/

            console.log('Dwindle Down Count : '+roomChatCount); // TODO uncomment this line
            room.chatCount = parseInt(roomChatCount)+1;// TODO-Imp room.chatCount replced with roomChatCount - 15 dec 2015
            var us= room.users;
            if(us){
    ...
    //                        console.log(socketClientId);
            ...
            else if(userStatus != status_playing || userStatus != status_play)//Start - Get user current status - anas - 15 dec 2015
            {
