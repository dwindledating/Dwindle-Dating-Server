...
var pic_path = "http://159.203.245.103:3000/";
...
//    var sockets =[];
    ...
    io.sockets.on('connection', function(socket){
        ...
        socket.on('chat', function (username,to_username) { // TODO Imp - anas - Remove ',status' third parameter required by client on 15 Feb 2016 
            ...
            from_user=username;
            to_user=to_username;
            //chat_status=status; // TODO Imp - anas - Remove status required by client on 15 Feb 2016 
            var chatJSON= new Object();
            var chat_array1=new Array();
            var chat_array=new Array();
            var chat_json;
            var count=1;
            //if(connection===null) { // Commented bcoz it was causing issue for db connection lost - anas - ssTODO-UP - 20 Jan 2016
                connection = sqlConnection.handleDisconnect();
            //}
            //start - Get chat room main user name for in app push notification - 13 dec 2015 - anas
            var query = "select name from users where user_name = "+ from_user;
                                 connection.query(query, function (err, rows, fields) {
                                        if (!err) {
                                            if(rows.length>0)
                                            {
                                                chatUserName = rows[0].name;
                                                console.log('chat room main UserName = '+chatUserName);
                                            }
                                        }
                                    });
            //End
                    ...
                    connection.query(query, function (err, rows, fields) {
                        if(!err){
                            for(i in rows){
                                ...
                                chat_json['Message']= mysql_real_escape_string(rows[i].text); //17 Feb 2016 - 'mysql_real_escape_string(' added - Fixation emojis issue
...
//                  chat_array['RoomName']=from_user+"-"+to_user;
                        ...
                        funGetToUserPics(to_username,function(pic_json){
                            findUserRoom(1,from_user,to_user,function(resRoom) {
                                ...
                                if(roomname){
                                    /* TODO Imp Comeneted on 15 Feb 2016  -- //Start - 29 Jan 2016 - Fixing chat event fire twice from client side issue - anas
                                    var socketRoomName = findRoomInSocket(roomname,io.sockets.connected);
                                    if(socketRoomName)
                                    {
                                        console.log('Chat ----> RoomName = '+socketRoomName+' already exist in socket.chatRoom')
                                    }
                                    else 
                                    {
                                        console.log('Chat ----> Creating room in socket.chatRoom')
                                    // */
                                        socket.chatRoom=roomname;// TODO-Imp - 'socket.room' updated with 'socket.chatRoom' - 17 dec 2015
...
//                  chat_array['RoomName']=from_user+"-"+to_user;
        ...
        socket.on('sendchat_chatlog', function (data) {
                console.log('sendchat_chatlog called = '+socket.chatRoom);// TODO-Imp - 'socket.room' updated with 'socket.chatRoom' - 17 dec 2015
                var date = new Date();
...
//                        console.log(socketClientId);
                    ...
                    var isOtherUserInChatRoom=false;
                    
                    for(var i=0;i<usersAttendingInRoom.length;i++)
                    {
                        if(usersAttendingInRoom[i].username===to_user)
                        {
                            isOtherUserInChatRoom = true;
                            console.log('User usersAttendingInRoom = ' +usersAttendingInRoom[i].username+' Flag isOtherUserInChatRoom = '+ isOtherUserInChatRoom);
                    ...
                    if (!isOtherUserInChatRoom) // Fixing - Duplicate message issue - anas - 18 Feb 2016
                    {
                        // start - throw in app notification if user is not on screen but available online ---  anas - 6 Oct 2015
                            //Start - TODO-Imp - 19 Dec 2015
                            var userStatus;// = getUserCurrentStatus(toUser);
                            getUserCurrentStatus(to_user,function(cRes){ 
                                console.log('cRes : '+cRes); 
                                userStatus = cRes;
                            });
                            chatSentChat(data,to_user,userStatus,fullDate,fullTime);
                            //End
                    };
...
//                }
        ...
        socket.on('leave_room',function(){
            console.log('leave room called named : '+ socket.chatRoom);
            socket.leave(socket.chatRoom);

        }); //End

        /* Commented on 17 Feb 2015 - because it is deleting the other user socket if app quite from play screen
        socket.on('disconnect',function(){
            ...
            //socket.leave(roomname); // 29 Jan 2016
            socket.leave(socket.chatRoom); // 29 Jan 2016
            socket.disconnect();

        }); //End of socket disconnect event*/

        // 17 Feb 2016 - anas - Fixation - double message issue in chat
        socket.on('leaveChatRoom',function(){
            delete user[socket.username];
            console.log("On leaveChatRoom -----> socket.chatRoom : "+socket.chatRoom+" and socket.username = "+socket.username);
            socket.leave(socket.chatRoom);
            //socket.disconnect();
        }); //End of leaveChatRoom event
...
//                console.log(result);
                console.log('Chat -> ----- loggedout --------> socket.leave(roomname); called');// 
                delete user[socket.username];
                socket.disconnect();
       ...
       //TODO-Imp - 19 Dec 2015
       function chatSentChat(data,to_user,userStatus,fullDate,fullTime)
       {
                            ...
                            if (mainSocket) {
                                console.log('Chat ------> fetch user status : '+ userStatus);
                                if(userStatus != 'chat')// TODO-Imp - 19 Dec 2015
                                {   
                                    // 22 Jan 2016 -Change Full to first name- required in Other issues doc: DwindleNotes11816.pdf
                                      if(chatUserName !== null || chatUserName !== "undefined" || chatUserName !== "")
                                      {
                                          var userFullNameArr =chatUserName.split(" ");
                                          chatUserName=userFullNameArr[0];
                                          console.log('Chat --------------> User Firsr name : '+ chatUserName);
                                      }
                                      //

                                //'message_from_matches_screen', 'Event : message_from_matches_screen          Message : ' +
                                    console.log('Chat Screen -------------> message_from_matches_screen ----------->'+chatUserName);
                                    mainSocket.emit('message_from_matches_screen',data, from_user, 'MatchesScreen', chatUserName, 'chat');// chatUserName, 'chat' added on 13 De 2015 // TODO-Imp remove 'Event : message_from_matches_screen          Message : ' 
                                }
                                msgStatus="read";
                                insertChatLogToDB(1, from_user, to_user, fullDate,fullTime ,data, msgStatus,roomname,function (getResult) {
                                });
                            }//end - anas - 7 Oct 2015
                            else //if(userStatus == 'loggedoff')// TODO-Imp - 19 Dec 2015 - check added for testing purpose
                            {
                                ...
                                query.find({
                                    success: function(results) {
                                        ...
                                        if(UserObjID!="") {
                                            var userFullNameArr =chatUserName.split(" ");// 24 Feb 2016 - Send only user first name 
                                            chatUserName=userFullNameArr[0];
                                            console.log("Chat ---------------> UserObjID: "+UserObjID + " ------ chatUserName = "+ chatUserName);
                                                        
                                            var query = new Parse.Query(Parse.Installation)
                                                , parser_                                = {
                                                    "alert": chatUserName+": "+data,// 'from_user' replaced with 'chatUserName'  
                                                    "fromUserFbId": from_user, // 'anotherObjectId' replaced with 'fromUserFbId' -- extra data to send to the phone. // 
                                                    "playEvent": 0,
                                                    "fromUserName":chatUserName,//15 Feb 2016 - required at front end
                                                    "sound": "cheering.caf" // default ios sound.
                                                };
                                            query.equalTo("user", {"objectId": UserObjID /* a user object id */, "className": "_User", "__type": "Pointer"}); // me.
                                            query.equalTo("deviceType", "ios");
...
var insertChatLogToDB=function(count, fromUser, toUser, fullDate,fullTime ,data,msg_Status,roomName,cb) {
    ...
    console.log('Chat ----> chat_log ----- message : '+ message);
    var strSql = "insert into chat_log(from_userid,to_userid,text,date,time,status,roomname) values('" + fromUser + "','" + toUser + "','" + message + "','" + fullDate +" " +fullTime+"','"+fullTime+"','"+msg_Status+"','"+roomName+"')";
    //console.log(strSql);
    connection.query(strSql, function (err1, result1) {
        if (!err1) {
            ...
            console.log('Chat --- insertChatLogToDB ----> sqlConnection.closeConnection Error : '+ err1);
        ...
        //start - 3 Jan 2016 - Convert special character string
...
var findRoomInSocket = function(roomName, sockets){
    for(var socketKey in sockets){
        var socketObj = sockets[socketKey];
        //console.log('findRoomInSocket - socketObj = '+sockets[socketKey]);// Comment this lines
        if(socketObj.chatRoom === roomName){
            console.log('findRoomInSocket - socketObj.chatRoom = '+socketObj.chatRoom);// Comment this line
            return socketObj;
        }
    }
    return null;
...
var getUserCurrentStatus = function(user_fb_id,resCallBack) // 17 Feb 2016
    {
      var userStatus;
            if(connection===null){
                    connection = sqlConnection.handleDisconnect();
                    ...
                    connection.query(query,function(err,rows,fields){
                       if(!err){
                           for (var i in rows) {
                               ...
                               console.log('Chat ----> Second User status = '+ userStatus);
