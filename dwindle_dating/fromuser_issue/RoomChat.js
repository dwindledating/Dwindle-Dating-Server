...
//     io.of('/').on('connection', function (socket) {
    ...
    io.sockets.on('connection', function (socket) {
      ...
      socket.on('login-temp', function (user_facebook_id) {
          ...
          var query = "update user_status set status = 'loggedin',isPlaying = 1 where user_id='" + facebook_id + "'";
          console.log(query);
          connection.query(query, function (err, rows, fields) {
              if (!err) {
                  response_JSON = {
...
//                console.log("Finding user according to criteria...");
                ...
                query += " and age >=" + userRequirement['Required_From_Age'] + " and age <=" + userRequirement['Required_To_Age'];
                query += " AND `user_status`.`user_id` NOT IN (SELECT `blocked_user_id` FROM (`blocked_user`))";//added on 19 Dec 2015
                query += " HAVING distance <" + userRequirement['Distance'];
                                        ...
                                        connection.query(query,function(err,rows,field){
                                            if (rows.length === 0) {
                                                ...
                                                //if (userSocket) {
                                                    ...
                                                    console.log("Play Request ------> Second User Name: "+secUser_Name);
                                                    if(secUser_Name){
...
//                                               console.log(sec_user + " Matched");
                                                    console.log('Play Request ------> firstUser = '+firstUser);
...
//                            console.log(dir);
                        ...
                        if(usrSocket){
                            console.log('Called ------------------------------------> dwindledown');
...
//                        console.log(JSON.stringify(userInfo));
                        ...
                        var usrSocket = findSocket(mainUser.name,io.sockets.connected);
                        if(usrSocket){
                            console.log('Called ------------------------------------> dwindledown2');
                            usrSocket.emit('dwindledown', JSON.stringify(dwindle_down));
                        }
        ...
        socket.on('leaveGame', function (fb_id) { // 19 dec 2015 - 'loggedout' rename with 'leaveGame'
            ...
            if(room){
                ...
                if(us){
                    ...
                    updateIsPlayingFlag(othUser, 0);// TDODO-Imp 21 dec 2015 - anas
                    var soc = findSocket(othUser, io.sockets.connected);
                    if(soc){
                        if(room.dwindleCount){
                            console.log('leaveGame -------------------------------> room.dwindleCount = '+room.dwindleCount);
                            if(room.dwindleCount > 2  || room.dwindleCount == -1){// 'room.dwindleCount===4' Replace with 'room.dwindleCount > 2' - anas - 11 Jan 2016
...
//                            var us = room.users;
                    ...
                    if(soc){
                        ...
                        console.log('On logout ->  soc.leave(socket.room); called ...'); //TODO
                    }
...
//                }
    ...
    function getUserCurrentStatus(user_fb_id)
    {
      var userStatus;
            if(connection===null){
                    connection = sqlConnection.handleDisconnect();
