...
//     io.of('/').on('connection', function (socket) {
    ...
    //                                    console.log("arguments", arguments);
                    ...
                    else if( response_to_user_status == status_loggedIn || response_to_user_status == status_play || response_to_user_status == status_chat) // TODO-Imp ' || response_to_user_status == status_chat' added on 18 Dec 2015 
                    {
                ...
                connection.query(query, function (err, rows, fields) {
                    ...
                    if (!err) {
                        ...
                        if (rows.length > 0) {
                            ...
                            query += " and age >=" + userRequirement['Required_From_Age'] + " and age <=" + userRequirement['Required_To_Age'];
                            query += " and `status` = 'play' OR `status` = '"+status_loggedIn+"' OR `status` = '"+status_chat+"'";// TODO-Imp  (OR `status` = '"+status_chat+"'") added on 18 dec 2015 --- // OR `status` = '"+status_loggedIn+"'"//"previously :  and `status` = 'play'";  - Query updated - anas - 9 oct 2015
                            query += " HAVING distance <" + userRequirement['Distance'];
                            query += " ORDER BY distance";
...
//                console.log(socket.username+" connected to "+roomname);
        ...
        //                        console.log(socketClientId);
                ...
                if(isToUserInPlayScreen == false)
                {
                    ...
                    //console.log('Play Screen - mainSocket = '+ mainSocket.username+' to_username = ' + toUserName);// comment this line
                            if (mainSocket) {
                                mainSocket.emit('message_game_started', 'Event : message_game_started ----------- Message : Game Started ',fromUser,'PlayScreen'); // TODO-Imp
                            }
                            else 
                            {
                ...
                else if(isSecondUserInPlayScreen == false)
                {
                    ...
                    var mainSocket = findSocket(toUserName, io.sockets.connected);
                            if (mainSocket) {
                                mainSocket.emit('message_game_started', 'Event : message_game_started ----------- Message : Game Started for second user',fromUser,'PlayScreen'); // TODO-Imp
                            }
                            else 
                            {
    ...
    //                        console.log(socketClientId);
            ...
            else if(userStatus != status_playing || userStatus != status_play || userStatus == 'chat')//Start - Get user current status - anas - 15 dec 2015
            {
