...
//var save_directory = "/home/ubuntu/dwindle_dating/public/uploadedImages/";//AWS Live - TODO -UnComment this line before deployment

//Start - Updated on 29 Feb 2016 because set all environemnt dependent variable from single location 
var save_directory = "/root/dwindle_dating/public/uploadedImages/";
var pic_path1 = "http://159.203.245.103:3000/";
//var save_directory = require('./EnvironmentVaribles').save_directory;// //Digital Ocean Live = "/root/dwindle_dating/public/uploadedImages/" - TODO -UnComment this line before deployment
//var pic_path1 = require('./EnvironmentVaribles').domain_ip;//"http://159.203.245.103:3000/";
//End 
...
var status_chat = "chat";// 18 Dec 2015
...
var play = function (io) {
    ...
    var secUserJson;
    // 11 Feb 2016
    var game_request_second_user_fbId;
    var game_request_count;
    //
//    var sockets =[];
...
//     io.of('/').on('connection', function (socket) {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        // Update isPlaying status 19 Dec 2015
        socket.on('event_change_isPlaying_flag', function(user_fb_id, flag){
            updateIsPlayingFlag(user_fb_id, flag);
        });
        // Generic method for change status - anas - 13 Dec 2015
         socket.on('event_change_user_status', function(user_fb_id, status){
            ...
            console.log('event_change_user_status called with user_fb_id = '+ user_fb_id +' and status = '+status);
             if(connection===null){ //TODO-UP - Commented on 20 Jan 2016 //Uncommented on 13 Feb 2016 because long log file
        ...
        socket.on('connect with socket', function(username){
            ...
            //if(connection===null){ TODO-UP - 20 Jan 2016
                        connection = sqlConnection.handleDisconnect();
                //}
                var sql = "update user_status set status = 'loggedin' where user_id='" + username + "'";
                connection.query(sql, function (sqlerr, result) {
                    if (sqlerr) {
      ...
      socket.on('login-temp', function (user_facebook_id) {
          ...
          //if(connection===null){ TODO-UP - 20 Jan 2016
            console.log('connection : '+ connection);
              connection = sqlConnection.handleDisconnect();
          //}
          // ,isPlaying = 1 added jus for testing purpose
          var query = "update user_status set status = 'loggedin' where user_id='" + facebook_id + "'";//,isPlaying = 1
          console.log(query);
          connection.query(query, function (err, rows, fields) {
              if (!err) {
                  response_JSON = {
      ...
      //End
        ...
        socket.on('getUsersList', function(){
                console.log('getUsersList called -- with save_directory = '+save_directory +' pic_path1 = '+pic_path1);
        ...
        socket.on('apnsResponse', function(username,respondTo,userLat,userLon){
        ...
        var getUserStatus = function(response_to)
        {
            ...
            var response_to_user_status;
            var isPlaying;// 19 dec 2015
            if(connection===null){
                    connection = sqlConnection.handleDisconnect();
                }
            var query = "select status, isPlaying from user_status where user_id='"+response_to+"'";
                    connection.query(query,function(err,rows,fields){
                       if(!err){
                           for (var i in rows) {
                               //console.log('rows[i].status = '+ rows[i].status)
                               response_to_user_status=rows[i].status;
                               isPlaying=rows[i].isPlaying;// 19 dec 2015
                           }
                    ...
                    if(response_to_user_status == status_loggedOff)
                    {
                        ...
                        Parse.initialize(
                                ...
                                var UserObjID="";
                                //var anotherUserObjID = "";// 21 Dec 2015
                                ...
                                query.find({
                                    success: function(results) {
                                        ...
                                        if(UserObjID!="") {
                                            console.log("APNS Response -----> OnjID: "+UserObjID);
                                            var query = new Parse.Query(Parse.Installation)
                                                , parser_                                = {
                                                    "alert": response_by+": "+data,
                                                    "fromUserFbId": response_by, // 'anotherObjectId' replaced with 'fromUserFbId' -- extra data to send to the phone. // 
                                                    "playEvent": 1, // 22 Dec 2015
    ...
    //                                    console.log("arguments", arguments);
                    ...
                    else if( response_to_user_status == status_playing && isPlaying == 1) // ' && isPlaying == 1' added on 19 Dec 2015
                    {
                        if (mainSocket) {
                    ...
                    else if( (response_to_user_status == status_loggedIn || response_to_user_status == status_play || response_to_user_status == status_chat)  && isPlaying == 0) // ' && isPlaying == 0' added on 19 Dec 2015 // TODO-Imp ' || response_to_user_status == status_chat' added on 18 Dec 2015 
                    {
                        console.log('User is online and not playing game ---> start game');// comment this line
        ...
        socket.on('Play', function (username, usrLon, usrLat, _page) {
            ...
            deleteAllGameRequestFlag(1,username,function(cRes){ // 8 Feb 2016
              //console.log(cRes); 
            });

            //socket.game_request_count = 0; // TODO-UP 31 Jan 2016 - 
            game_request_count = 0;//11 Feb 2016
            play(username, usrLon, usrLat,_page);
        });//End Of Socket.on Play event

        //Start - Game Request Response Yes - // 31 Jan 2016 
        socket.on('game_request_response_yes', function (secondUser,secUser_Name,firstUser) {//secondUser,secUser_Name,firstUser,user_Lat,user_Lon
            //socket.game_request_count = 0;  //TODO-UP 
            game_request_count = 0;//11 Feb 2016

            // 'us.status' added on 12 March 2016 to Fixaction issue - if main user initiate game request and quite the app within 10 sec
            var query = "select u.name,up.user_name,us.user_id,us.status,req_gender,req_from_age,req_to_age,distance, up.pic_name,up.pic_path from user_status us, user_pics up,users u";
                query += " where us.user_id='" + firstUser + "' and up.user_name ='" + firstUser + "' and u.user_name='"+firstUser+"' limit 1";
                //console.log(query);
                connection.query(query, function (err, rows, fields) {
                    main_user['fb_id'] = firstUser;
                    if (!err) {
                        if(rows.length>0){
                            if (rows[0].status == status_play) // 'us.status' added on 12 March 2016 to fix issue - if main user initiate play request and quite the game
                            {
                                ...
                                if (rows[0].name) {
                                    ...
                                    console.log("Play Game Request ----------> MainUserName = " + mainUserName);
                                ...
                                isBothUserCriteriaMatch(secondUser, firstUser, secUser_Name);// username replace with user_name -  anas - 9 Oct 2015
                            }
                            else {
                                console.log("Game  Request yes ------> But Initiater (Main User) quite the app."); //Comment this line                                       
                                var userSocket = findSocket(secondUser, io.sockets.connected);
                                if (userSocket) {
                                    userSocket.emit('message_not_found', 'Event : message_not_found         Message : Sorry, No user found.');
                                }
                            }
                        }
                    }else{
        ...
        //Start - Game Request Response No - // TODO-UP 31 Jan 2016
        socket.on('game_request_response_no', function (secondUserFbid,mainUserFbid, usrLon, usrLat) { //secondUser,secUser_Name,firstUser,user_Lat,user_Lon ---- // 31 Jan 2016 ------ 'request_count' added on 11 Feb 2016 
            var mainSocket = findSocket(mainUserFbid, io.sockets.connected);// 10 Feb 2016
            if (mainSocket) {
                  mainSocket.emit('event_no_against_game_request', 'Other user has rejected the game request. Please reset counter.');
            }
            console.log('Test ========== game_request_second_user_fbId : '+ game_request_second_user_fbId);// TODO-Imp comemnt this line , it is jus for testing purpose
            setGameRequestFlag(1,mainUserFbid,secondUserFbid,function(cRes){ // 8 Feb 2016
              //console.log(cRes); 
            });
            
             setGameRequestFlag(1,secondUserFbid,mainUserFbid,function(cRes){
              //console.log(cRes); 
            });
            game_request_count = game_request_count +1; // 11 Feb 2016
            play(mainUserFbid, usrLon, usrLat,0); //request_count                                      
        });//End 

        // 10 Feb 2016 
        socket.on('game_request_ignored', function (mainUserFbid, usrLon, usrLat) { //secondUser,secUser_Name,firstUser,user_Lat,user_Lon ---- // 31 Jan 2016 ------ 'request_count' added on 11 Feb 2016 
            
            var userStatus = '';
            getUserCurrentStatus(mainUserFbid,function(cRes){ 
                console.log('cRes : '+cRes); 
                userStatus = cRes;
            });
            console.log('Play ---> game_request_ignored --->  Current User status : '+userStatus); 
            if(userStatus == status_play) // 13 Feb 2016
            {
              var mainSocket = findSocket(mainUserFbid, io.sockets.connected);// 10 Feb 2016
              if (mainSocket) {
                    mainSocket.emit('event_no_against_game_request', 'Other user has rejected the game request. Please reset counter.');
              }
              console.log('Event ---- game_request_ignored called => game_request_second_user_fbId : '+ game_request_second_user_fbId);
              setGameRequestFlag(1,mainUserFbid,game_request_second_user_fbId,function(cRes){ // 8 Feb 2016
                //console.log(cRes); 
              });
              
               setGameRequestFlag(1,game_request_second_user_fbId,mainUserFbid,function(cRes){
                //console.log(cRes); 
              });
              game_request_count = game_request_count +1; // 11 Feb 2016
              play(mainUserFbid, usrLon, usrLat,0); //request_count                          
            ...
            {
              console.log('game_request_ignored event fired from client side against no play/Game request.');
              io.sockets.emit('event_Error', 'game_request_ignored event fired from client side against no play/Game request.');
            }
            ...
            {
                firstUserBoolean=true;// 21 Dec 2015
                ...
                from_user = user_name;
                // 11 Feb 2016
                game_request_second_user_fbId = ''; 
                //
                console.log("Play called - User: "+user_name+", LAT:"+user_Lat+", LON:"+user_Lon, "Page:"+page);
                //End

                //socket.username = username; // 18 Dec 2015 - handled in connect with socket event
                console.log("play ----> Socket.username ="+socket.username);
                ...
                var sql = "update user_status set status = 'play',location = POINT(" + usrLon + "," + usrLat + "), isPlaying=0 where user_id='" + username + "'";//, isPlaying = 0 added on 23 Dec 2015
                connection.query(sql, function (sqlerr, result) {
                    if (sqlerr) {
    ...
    //                        console.log(path);
                            if(rows[0].name){
                                var mainUserFullName=rows[0].name;
                                mainUserName = mainUserFullName; // get user for apns response user call to criteria function - anas - 4 oct 2015
                                console.log("Play ----------> MainUserName = " + mainUserName);
                                var mainUserName_Array=mainUserFullName.split(" ");
                                main_user['user_name']=mainUserName_Array[0];
                            }
                            else{
                    ...
                    if (!err) {
                        ...
                        if(rowsLength > 0)
                        {
                            ...
                            if(isAPNSResponsePlayRequest === false)
                            {
                                ...
                                for (var i in rows) {
                                    if(rows[i].status == status_loggedIn || rows[i].status == status_play  || rows[i].status == status_chat) // TODO-Imp ' || response_to_user_status == status_chat' added on 18 Dec 2015  
                                    { //not needed handled in Query updated - where clause added - anas - 9 Sep 2015
                                ...
                                //End
                                 ...
                                 console.log('Play ------> game_request_count : ' + game_request_count); //  11 feb 2016
                                if(online_user_rowsLength > 0 && game_request_count < 4)
                                {
                                    ...
                                    for(var i in rows){
                                        ...
                                        if (rows[i].status == status_loggedIn || rows[i].status == status_play  || rows[i].status == status_chat) // TODO-Imp ' || response_to_user_status == status_chat' added on 18 Dec 2015  
                                        {  //Fill only online user list found in preferences - 9 Sep 2015 - anas
                                               if(count<=5) {
                                        ...
                                        {       
                                                ...
                                                if (userSocket) {
                                                    // 11 Feb 2016
                                                    /*var gameRequestCount = getGameRequestCountSocket(main_user, io.socketsconnected);//socket.game_request_count;
                                                    socket.game_request_count = gameRequestCount + 1;  // 31 Jan 2016
                                                    console.log("Play ------------> Second user is Connected with socket --- game_request_count = "+socket.game_request_count); TODO-UP */
                                                    
                                                    //Start - If 2 users pressed play at the same time then initiate a game without sending game request - this requirement was requested on 24 Feb 2016 - updated by anas
                                                    // Code commented required on 24 Feb 2016 - its Creating new use case
                                                    /*if(rows[i].status == status_play)
                                                    {
                                                        console.log('Play ----------> Other user status = '+ rows[i].status);
                                                        isBothUserCriteriaMatch(rows[i].user_id, user_name,rows[i].name);// username replace with user_name -  anas - 9 Oct 2015
                                                        break;
                                                    }
                                                    else
                                                    {*/
                                                    //End
                                                        var userFullNameArr =mainUserName.split(" ");//24 Feb 2016 - Only send first name
                                                        mainUserName=userFullNameArr[0];

                                                        //Start - // 31 Jan 2016 - Ask second user is he/sher interested in play or not...
                                                        console.log('Play screen ------> game_request called.');
                                                        game_request_second_user_fbId = rows[i].user_id; // 11 Feb 2016
                                                        console.log('Play ----> game_request_second_user_fbId : '+ game_request_second_user_fbId+ ' and game_request_count = '+ game_request_count);
                                                        userSocket.emit('game_request', mainUserName + ' wants to play with you. Are you interested in play?',rows[i].user_id,rows[i].name, user_name,user_Lat,user_Lon);//secondUser,secUser_Name,firstUser,user_Lat,user_Lon,game_request_count
                                                        //End
                                                        //isBothUserCriteriaMatch(rows[i].user_id, user_name,rows[i].name);// username replace with user_name -  anas - 9 Oct 2015
                                                        break;
                                                    //}
                                ...
                                else if((offline_user_rowsLength == rowsLength) || (offline_user_rowsLength >0 && game_request_count > 3)) // || (offline_user_rowsLength >0 && game_request_count > 3) - added on 11 Feb 2016
                                {
                                            ...
                                            success: function(results) {
                                                ...
                                                for(var index =0 ; index < offline_user_rowsLength;  index++ )
                                                {
                                                    ...
                                                    for(var a=0;a<results.length;a++){
                                                        ...
                                                        /*if(UserObjID == "" || anotherUserObjID == "")// Set another user object Id ---- 21 Dec 2015
                                                        {*/
                                                          ...
                                                          /*else if(from_user===results[a].attributes.username){
                                                            anotherUserObjID = results[a].id; //;    //TODO Muhammad yunas : S8eatsJ2O3
                                                            //break;
                                                          }
                                                        }
                                                        else
                                                     ...
                                                     if(UserObjID!="") {
                                                        console.log("Play ---------------> UserObjID: "+UserObjID);
                                                        var userFullNameArr =mainUserName.split(" ");// 24 Feb 2016
                                                        mainUserName=userFullNameArr[0];
                                                        var query = new Parse.Query(Parse.Installation)
                                                            , parser_data = {
                                                                "alert": "Oh snap! Someone wants to chat with you!",// updated on 13 Jan 2016 //mainUserName+" is waiting for you to connect.",//+data 
                                                                "fromUserFbId": from_user, // 'anotherObjectId' replaced with 'fromUserFbId' -- extra data to send to the phone. // 
                                                                "playEvent": 1, // 22 Dec 2015
                                                                "fromUserName":mainUserName,// 15 Feb 2016 - required at front end
                                                                "sound": "cheering.caf" // default ios sound.
                                                            };
                                                        query.equalTo("user", {"objectId": UserObjID /* a user object id */, "className": "_User", "__type": "Pointer"}); // me.
                                                        query.equalTo("deviceType", "ios");
                                                        ...
                                                        Parse.Push.send({
                                                            ...
                                                            success: function () {
                                                                ...
                                                                UserObjID= "";
                                                                console.log("Play - Push Notification Sent Successfully - Make UserObjID Empty for next user = "+ UserObjID);
                                                            //  console.log("arguments", arguments);
                                                            },
                                                            error: function (error) {
                                                    ...
                                                    {
                                                      console.log('Play : User = '+to_user+' Parser Object not exist');
                                                    }
                            ...
                            else
                            {
                                ...
                                if (userSocket) {
                                    ...
                                    //Start- Add second user name against APNS request - anas - 23 Dec 2015
                                    if(connection===null){
                                        connection = sqlConnection.handleDisconnect();
                                    }
                                    var query = "Select name FROM users where user_name = "+response_to_user;
                                    connection.query(query, function (err, rows, fields) {
                                        if (!err) 
                                        {
                                            if(rows.length>0)
                                            {
                                                var second_user_name= "";
                                                for (var i in rows) {
                                                      second_user_name = rows[i].name;
                                                    }

                                                    sec_user['fb_id'] = response_to_user;
                                                    console.log("APNS Response ---------> Second User Name: "+second_user_name);
                                                    if(second_user_name){
                                                        var secondUsrFullName=second_user_name.split(" ");
                                                        sec_user['user_name']=secondUsrFullName[0];
                                                        secondUserName =secondUsrFullName[0];// 3 Jan 2015 -- Fixing second user name issue against - apns response - 3 Jan 2016
                                                    }
                                                    else{
                                                        sec_user['user_name']="";
                                                        secondUserName ="";
                                                    }
                                                    console.log('ApnsResponse Play Request ----------> 1. Response By = '+response_by);
                                                    getChatUsersPicturesName(sec_user, response_by);// 'response_by' replaced with 'user_name' - fixing second user name issue against - apns response - 3 Jan 2016
                                            }
                                            else
                                            {
                                                 sec_user['user_name']="";
                                        ...
                                        {
                                            console.log('Error fetching second users name against APNS response.');
                                        }
                                ...
                                //isAPNSResponsePlayRequest = false;// Comented on  2 Jan 2016 -Fixing second user name against apns game play respons request//Fixing through APNS flag true application start chatting with same user -- anas 12 dec 12
                            }
                        ...
                        else
                        {
                            ...
                            var main_user_ = user_name;//resJSON["MainUser"].fb_id;
                                            ...
                                            if(rows.length>0)
                                            {
                                                ...
                                                for (var i in rows) {
                                                    ...
                                                    if(second_user_name){
                                                        ...
                                                        sec_user['user_name']=secondUsrFullName[0];
                                                        secondUserName =secondUsrFullName[0];// 3 Jan 2015 -- Fixing second user name issue against - apns response - 3 Jan 2016
                                                        console.log("APNS Response ---------> Second User Name: "+second_user_name+ " and sec_user['user_name'] = "+sec_user['user_name']);
                                                    }
                                                    else{
                                                        sec_user['user_name']="";
                                                        secondUserName ="";// 3 Jan 2015 -- Fixing second user name issue against - apns response - 3 Jan 2016
                                                    }
                                                    console.log('ApnsResponse Play Request ----------> 2. Response By = '+response_by);
                                                    getChatUsersPicturesName(sec_user, response_by);// 'response_by' replaced with 'user_name' - fixing second user name issue against - apns response - 3 Jan 2016
                                            }
                                            else
                                            {
                                ...
                                //isAPNSResponsePlayRequest = false;//Comeneted on  2 Jan 2016 - Fixing through APNS flag true application start chatting with same user -- anas 12 dec 12
                            }
                            //End
                            else
                            {
                ...
                connection.query(query, function (err, rows, fields) {
                    ...
                    if (!err) {
                        ...
                        if (rows.length > 0) {
                            ...
                            query += " and age >=" + userRequirement['Required_From_Age'] + " and age <=" + userRequirement['Required_To_Age'];
                            query += " and `status` = 'play' OR `status` = '"+status_loggedIn+"' OR `status` = '"+status_chat+"' AND isPlaying = 0";//  'AND isPlaying = 0' added on 19 Dec 2015 // TODO-Imp  (OR `status` = '"+status_chat+"'") added on 18 dec 2015 --- // OR `status` = '"+status_loggedIn+"'"//"previously :  and `status` = 'play'";  - Query updated - anas - 9 oct 2015
                            query += " AND `user_status`.`user_id` NOT IN (SELECT `blocked_user_id` FROM (`blocked_user`) where user_id='"+user_name+"')";//added on 19 Dec 2015 // updated on 14 Jan 2016 - TODO-Imp (where user_id='"+user_name+"') added
                            query += " AND `user_status`.`user_id` NOT IN (SELECT `user_id` FROM (`blocked_user`) where blocked_user_id='"+user_name+"')";//added on 19 Dec 2015 // updated on 14 Jan 2016 - TODO-Imp ( where blocked_user_id='"+user_name+"') added
                            query += " HAVING distance <" + userRequirement['Distance'];
                            query += " ORDER BY distance";
                                        ...
                                        connection.query(query,function(err,rows,field){
                                            if (rows.length === 0) {
                                                ...
                                                //if (userSocket) {
                                                    ...
                                                    console.log("Against Play Request ------> Second User Name: "+secUser_Name);
                                                    if(secUser_Name){
...
//                                               console.log(sec_user + " Matched");
                                                    console.log('Play Request initiator ------> firstUser = '+firstUser);
                                                    getChatUsersPicturesName(sec_user, firstUser);
                                               // }

                                            }
                                            else
                                            {// 14 Jan 2016 - TODO-Imp
                                              console.log('Users are blocked each other or made friends before.');
                                            }
                                        });
            ...
            var getChatUsersPicturesName = function (sec_user,firstUser) {
                ...
                //console.log(query);
                var usrJson;
                if(isAPNSResponsePlayRequest == true)// 3 Jan 2016 - Fixing apns response - second user name issue
                {
                  console.log("APNSResponsePlayRequest ------> Finding Second User Json ....");
                    usrJson = findUserJSon(firstUser);
                }
                else if(socket.username != firstUser)
                {
                    console.log('Play screen -- Finding user Json of : '+ firstUser);
                    usrJson = findUserJSon(firstUser);
                }
                else
                {
                    usrJson = findUserJSon(socket.username);
                }
...
//                var query = "Select DISTINCT(u.user_name) un, pic_name, pic_path from users u,user_pics up where u.user_name = up.user_name and u.gender = 'Female' and u.user_name!='"+secUsr+"' and u.user_name!='"+firstUsr+"'";
                ...
                console.log('Play --- get other users query : '+query);
                connection.query(query, function (err, rows, fields) {
                    if (!err) {
                        var count=1;
                        for (var i in rows) {
                            if(count<=4){
                                ...
                                user['pic_path'] = path;
                                console.log('Play ----- '+count+' other_users.push(user)');
                        ...
                        console.log('Play ----------------------> if(firstUserBoolean == '+firstUserBoolean+')');
                        if(firstUserBoolean){
            ...
            var funAddSecondUserInJSON= function(secUsr,firstUsr,other_users,callback){
                ...
                if(usrJSON){
                    ...
                    console.log('play -----> 1. RoomName ='+roomname);
                    ...
                    if (mainSocket) {
                        ...
                        //console.log(JSON.stringify(resJSON));
                            if (query_response === 'Success') {
                                socket.secondUserdata = JSON.stringify(resJSON);// TODO-Imp - 19 dec 2015 //room 
                                console.log('Second User JSON 2.0 : '+ socket.secondUserdata);
                                console.log('Play ----------------------> 2.0 startgame called ....'); // TODO
                                isAPNSResponsePlayRequest = false;// added on  2 Jan 2016 -Fixing second user name against apns game play respons request//Fixing through APNS flag true application start chatting with same user -- anas 12 dec 12
                    ...
                    connection.query(query, function (err, rows, fields) {
                        ...
                        var roomname = result_json['MainUser'].fb_id + "-" + result_json['SecondUser'].fb_id;
                        resJSON["RoomName"] = roomname;
                        console.log('play -----> 2. RoomName ='+roomname);
                        var main_user = resJSON["MainUser"].fb_id;
                        var mainSocket = findSocket(main_user, io.sockets.connected);
                        if (mainSocket) {
                            funAddUsersInfoToDB(1, main_user, resJSON, function (query_response) {
//                                        console.log(JSON.stringify(resJSON));
                                if (query_response === 'Success') {
                                    socket.secondUserdata = JSON.stringify(resJSON);// TODO-Imp - 19 dec 2015 //room
                                    console.log('Second User JSON : '+ socket.secondUserdata);
                                    console.log('Play ----------------------> 2.1 startgame called ....'); // TODO
                                    isAPNSResponsePlayRequest = false;// added on  2 Jan 2016 -Fixing second user name against apns game play respons request//Fixing through APNS flag true application start chatting with same user -- anas 12 dec 12
                                    mainSocket.emit('startgame', JSON.stringify(resJSON));
                                }
                            });
...
//                console.log("FIRST USER: "+JSON.stringify(result_json));
                ...
                resJSON["MainUser"]=usrJSON.MainUser[0];
                if(isAPNSResponsePlayRequest == true)// 3 Jan 2016 - Fixing second user name issue -- 
                {
                  if (usrJSON.SecondUser[0]) { // 14 Jan 2016 - TODO-Imp
                    usrJSON.SecondUser[0].user_name = secondUserName;
                    console.log('usrJSON.SecondUser[0].[user_name] '+ usrJSON.SecondUser[0].user_name + ' --- '+ secondUserName);
                  }
                }
                resJSON["SecondUser"]=usrJSON.SecondUser[0];
                resJSON["OtherUsers"]=usrJSON.OtherUsers[0];
                var roomname = result_json['MainUser'].fb_id+"-"+result_json['SecondUser'].fb_id;
                resJSON["RoomName"]=roomname;
                console.log('play -----> 3. RoomName ='+roomname);
...
//                                        console.log(JSON.stringify(resJSON));
                                if (query_response === 'Success') {
                                    ...
                                    console.log('User Json : '+JSON.stringify(resJSON));// TODO - 
                                    socket.firstUserdata = JSON.stringify(resJSON);// TODO-Imp - 19 dec 2015 //room
                                    console.log('Play ----------------------> 1. startgame called ....'); // TODO
                                    mainSocket.emit('startgame', JSON.stringify(resJSON));
            ...
            var funAddChatUserToDB= function(rec,result_json,other_users,cb){
                ...
                var query="select * from chat_room where room='"+result_json['RoomName']+"'";
                              //console.log('Play - result_json[RoomName] - query : '+query);
                connection.query(query, function (err, rows, fields) {
                    if (!err) {
                        if (rows.length > 0) {
                            console.log('Play screen - other_users.length = '+ other_users.length);
        ...
        // Imp - secondUser_fbId added as third parameter anas - 18 dec 2015
        socket.on('addUser', function (roomname,mainusername,secondUser_fbId){// TODO - mainusername added - anas - 11 Oct 2015) 

            console.log('Second User -----> secondUser_fbId '+ secondUser_fbId);
...
//                console.log(socket.username+" connected to "+roomname);
                ...
                var isSecondUserInPlayScreen = false;
                if(roomName)
                        {
        ...
        //                        console.log(socketClientId);
                            ...
                            for(var i=0;i<usersAttendingInRoom.length;i++){
                              console.log('usersAttendingInRoom[i].username = '+ usersAttendingInRoom[i].username + ' == SecondUser_fbId : '+secondUser_fbId);
                                    ...
                                    } // Imp- anas - 18 dec 2015
                                    else if(usersAttendingInRoom[i].username===secondUser_fbId){
                                        console.log('Second User : '+usersAttendingInRoom[i].username+' is within Play screen');
                                        isSecondUserInPlayScreen = true;
                                    }
                ...
                if(isToUserInPlayScreen == false)
                {
                    ...
                    //console.log('Play Screen - mainSocket = '+ mainSocket.username+' to_username = ' + toUserName);// comment this line
                            if (mainSocket) {
                              updateIsPlayingFlag(secondUser_fbId,1);
                                console.log('isToUserInPlayScreen  ----> Event message_game_started -----> Second User JSON : '+ socket.secondUserdata);// TODO-Imp - 19 Dec 2015
                                mainSocket.emit('message_game_started', 'Event : message_game_started ----------- Message : Game Started ',fromUser,'PlayScreen',socket.secondUserdata); // TODO-Imp - 19 Dec 2015 'socket.secondUserdata' added
                            }
                            else 
                            {
                ...
                //Start - Imp - anas - 18 Dec 2015
                else if(isSecondUserInPlayScreen == false)
                {
                    /*var user2 = {name:secondUser_fbId, chatCount:0}; //TODO-Imp
                    room1.users.push(user2);*/

                    /*var toUserName = secondUser_fbId;
                    console.log('Second User -----> secondUser_fbId = '+ toUserName+ ' name = '+ user2.name);
                    var mainSocket = findSocket(toUserName, io.sockets.connected);
                            if (mainSocket) {
                               updateIsPlayingFlag(secondUser_fbId,1);
                                console.log('isSecondUserInPlayScreen check ---> Event message_game_started');// -----> Second User JSON : '+ socket.secondUserdata);// TODO-Imp - 19 Dec 2015
                                mainSocket.emit('message_game_started', 'Event : message_game_started ----------- Message : Game Started for second user',fromUser,'PlayScreen', socket.secondUserdata); // TODO-Imp 19 Dec 2015
                            }
                            else 
                            {
                                    console.log('Second User :send push notification if user press back or quite the app.');
                                
                            }*/
                }//End
                else
                {
                  console.log('User within play screen');
                }
        ...
        socket.on('sendchat', function (data) {
            ...
            var room = findChatRoom(socket.room);
              ...
              console.log('Exception Room : '+err);
            }
            try
            {
            console.log('Dwindle Down Count : '+room.chatCount); // TODO uncomment this line
            }catch(err) {
              console.log('Exception ChatCount : '+err);
              room.chatCount = 0;
            }
            ...
            //Start - TODO-Imp 18 Dec 2015
            var userStatus;// = getUserCurrentStatus(toUser);
            if(connection===null){
                    connection = sqlConnection.handleDisconnect();
                } 
            var query = "select status from user_status where user_id='"+toUser+"'";
                    connection.query(query,function(err,rows,fields){
                       if(!err){
                           for (var i in rows) {
                               userStatus=rows[i].status;
                               console.log('current User status = '+ userStatus);
                               //return userStatus;
                           }

                          playSendChat(data,userStatus,roomName,fromUser,toUser,mainUser,user,room);//TODO-Imp 18 Dec 2015
                       }
                       else{
                            console.log('Error in fetching user status'); //comment this line
                            //return 'Error';
                       }
                     });
            //End
...
//                            console.log(dir);
                        ...
                        if(usrSocket){
                            console.log('Called ------------------------------------> dwindledown Json = '+JSON.stringify(dwindle_down));
...
//                        console.log(JSON.stringify(userInfo));
                        ...
                        var usrSocket = findSocket(mainUser.name,io.sockets.connected);
                        if(usrSocket){
                            console.log('Called ------------------------------------> dwindledown2 Json = '+JSON.stringify(dwindle_down));
                            usrSocket.emit('dwindledown', JSON.stringify(dwindle_down));
                        }
...
//                                            io.sockets.in(socket.room).emit('adduserspic', JSON.stringify(userPicInfo));
        ...
        //Start - 3 jan 2016 - empty play room completely
        var resetGame=function(){
            console.log('Play ------------------------------------------> resetGame start');
            if(connection===null) {
                connection = sqlConnection.handleDisconnect();
            }
            var sql = "update user_status set status = 'play', isPlaying=0 where user_id='" + socket.username + "'";//,isPlaying = 0 added on -- 23 Dec 2015 
            connection.query(sql, function (sqlerr, result) {
                if (!sqlerr) {
                                    ...
                                    deleteUserFromJsonDB(1,socket.room,socket.username,function(d_response){
                                        ...
                                        deleteUserFromJson(1,socket.room,othUsr,function(d_response){
                                            ...
                                            if(user_soc){
                                                ...
                                                console.log('On restartGamePlay -> user_soc.leave(socket.room) called ...'); //TODO
                                            }
                                            delete usernames[othUsr];
                                            delete usernames[socket.username];
                                            delete rooms[room];
                                            console.log('On restartGamePlay -> delete rooms[room] called ...'); //TODO
                                        });
                                    console.log('Play ------------------------------------------> resetGame End');
                                   });
        ...
        socket.on('skip',function(){
            ...
            if(room){
                ...
                var sql= "insert into blocked_user(blocked_user_id,user_id,isFriend) values('"+blockedUsr+"','"+skipUser+"',0)";// isFriend flag added on 7 Jan 2016
...
//                        console.log(strSql);
        ...
        socket.on('restartGamePlay', function (_user_fb_id, _usrLon, _usrLat) { // 23 Dec 2015 - '_user_fb_id, _usrLon, usrLat' added
            if(connection===null) {
                connection = sqlConnection.handleDisconnect();
            }
            // 'isPlaying = 0' added on 19 Dec 2015
            console.log('RestartGamePlay called -------->  with socket.username =' + socket.username+' -- LONG = '+_usrLon+' LAT :'+_usrLat);
            ///* Commented now deletion of rooms, status etc are handle against last dwindle down - 3 Jan 2016
            var sql = "update user_status set status = 'play', isPlaying=0 where user_id='" + socket.username + "'";//,isPlaying = 0 added on -- 23 Dec 2015 
            connection.query(sql, function (sqlerr, result) {
                if (!sqlerr) {
                    delete usernames[socket.username];
                                ...
                                funDeleteChatRoomFromDB(1, socket.room, function (delResult) {
                                    deleteUserFromJsonDB(1,socket.room,socket.username,function(d_response){
                                        ...
                                        deleteUserFromJson(1,socket.room,othUsr,function(d_response){
                                            ...
                                            if(room.dwindleCount){
                                                room.dwindleCount= -1;// 12 Jan 2016 
                                            }
                                            deleteChatHistory(1,socket.room, function(cres){
                                      ...
                                      socket.leave(socket.room); // 14 Jan 2016 - TODO-Imp - Fixation Issue: 'User 1 quite app and relauch again and then user 2 quite then user 1 got "disconnectResponse"'
                                      play(_user_fb_id, _usrLon, _usrLat,"0");// 23 Dec 2015 - Fire play against restart game 
        ...
        socket.on('disconnect',function(){
          console.log('Socket disconnect called'); // TODO uncomment this line - 15 dec 2015
            if(connection===null) {
                connection = sqlConnection.handleDisconnect();
            }
            // 11 Oct 2015 - anas - changes user status - 'loggedoff' ---------- // 'isPlaying = 0' added on 19 Dec 2015
            var sql = "update user_status set status = 'loggedoff', isPlaying = 0 where user_id='" + socket.username + "'";
            console.log('disconnect -----------> update status Query : ' + sql); // 17 Feb 2016
            connection.query(sql, function (sqlerr, result) {
                if (sqlerr) {
                    function_result = "User diconnect event query failed for user "+ socket.username;
            ...
            var othUsr='';
            var room = findChatRoom(socket.room);
            console.log('Disconnect ----------------> 0. socket.room = '+socket.room);
            if(room){
                var us = room.users;
                if(us){
                    if(us[0].name){
                ...
                //updateOtherUserStatus(othUsr,function(strResult){ 
                  updateOtherUserStatusForDisconnection(othUsr,function(strResult){ // Fixation issue - other user status changes when user quite and app - 25 Jan 2016 
                    /* if(room.dwindleCount){
                    ...
                    funDeleteChatRoomFromDB(1, socket.room, function (delResult) {
                        ...
                        if(us){
                            ...
                            console.log('Disconnect -------------> othUser : '+othUser);
                            var soc = findSocket(othUser, io.sockets.connected);
                            if(soc){
                                if(room.dwindleCount){
                                    console.log('Disconnect ------------------------------> room.dwindleCount = '+room.dwindleCount);
                                    if(room.dwindleCount > 2 || room.dwindleCount == -1){// 'room.dwindleCount===4' Replace with 'room.dwindleCount > 2 || room.dwindleCount == -1' - anas - 11 Jan 2016
                                   ...
                                   console.log('Else ---- Disconnect ------------------------------> room.dwindleCount = '+room.dwindleCount);
            ...
            console.log('Disconnect ----------------> 1. socket.room = '+socket.room);
            // below line uncomented at - 12 Jan 2016
            ...
            //socket.room = ''; // 12 Jan 2016
            console.log('Disconnect ----------------> 2. socket.room = '+socket.room);


        }); //End of socket disconnect event
        socket.on('leaveGame', function (fb_id) { // 19 dec 2015 - 'loggedout' rename with 'leaveGame'
            console.log('leaveGame called ...');
            updateIsPlayingFlag(fb_id, 0);// 19 dec 2015 - anas
            var room = findChatRoom(socket.room);
            if(room){
                ...
                if(us){
                    ...
                    updateIsPlayingFlag(othUser, 0);// TODO-Imp 21 dec 2015 - anas
                    var soc = findSocket(othUser, io.sockets.connected);
                    if(soc){
                        if(room.dwindleCount){
                            console.log('leaveGame -------------------------------> room.dwindleCount = '+room.dwindleCount);
                            if(room.dwindleCount > 3  || room.dwindleCount == -1){// 'room.dwindleCount===4' Replace with 'room.dwindleCount > 3' - anas - 11 Jan 2016


                            }
                            else{
                                //soc.emit('updatechat','SERVER: ',socket.username+' Loggedout');
                                soc.emit('leaveGameResponse',socket.username,'Leave the chat');// 19 dec 2015 - 'loggedoutResponse' rename with 'leaveGameResponse'
                            }
                        }
                        else{
                            console.log('Else ---- leaveGame ------------------------------> room.dwindleCount = '+room.dwindleCount);
                            //soc.emit('updatechat','SERVER: ',socket.username+' Loggedout');
                            if(room.dwindleCount >= 0)
                            {
                                soc.emit('leaveGameResponse',socket.username,'Leave the chat');// 19 dec 2015 - 'loggedoutResponse' rename with 'leaveGameResponse'
...
//            }
            ...
            var sql = "update user_status set status = 'loggedin', isPlaying = 0 where user_id='" + fb_id + "'";// 'socket.username' replace with 'fb_id'
            //console.log(sql);
...
//                            var us = room.users;
                            ...
                            updateOtherUserStatus(othUsr,function(strResult) {
                                ...
                                delete usernames[socket.username];
                                delete rooms[room];
                                console.log('On logout -> delete rooms[room] called ...'); //TODO
                            });
                    ...
                    if(soc){
                        ...
                        if (io.sockets.sockets[soc.id]) {
                            console.log("Inside");
                            //io.sockets.sockets[soc.id].disconnect();  // 19 Dec 2015 - anas - now never disconnect the user from socket
                        }
                        // 16 Feb 2016
                        if(room)
                        {
                            console.log('On leaveGame -----> room.userCount = '+ room.userCount);
                            room.userCount = 6; 
                        }
                        //
                        console.log('leaveGame ----------------> 1. socket.room = '+socket.room);
                        soc.leave(socket.room);
                        console.log('leaveGame ----------------> 2. socket.room = '+socket.room);
                        
                        //io.socket.manager.onClientDisconnect(soc.id);
                        //soc.disconnect(); // 19 Dec 2015 - anas - now never disconnect the socket
                        console.log('On leaveGame ->  soc.leave(socket.room); called ...'); //TODO
                    }
                    socket.leave(socket.room); // 14 Jan 2016 - TODO-Imp - Fixation Issue: 'User 1 quite app and relauch again and then user 2 quite then user 1 got "disconnectResponse"'
                }
            });
...
//                 console.log(query);
        ...
        var deleteUserFromJson=function(uVal,roomname,userName,uCback){
            ...
            console.log('Play ----------deleteUserFromJson------------------ firstUserBoolean = true');
        ...
        var deleteUserFromJsonDB=function(uVal,roomname,userName,uCback){
            ...
            console.log('Play ----------deleteUserFromJsonDB------------------ firstUserBoolean = true');
            firstUserBoolean=true;
            console.log("Play -------------- Emtying Array Main User: "+userName);
            emptyJSONfromArray(userName);// TODO 
            deleteUsrObjectFromJSON(userName);
            var sql="delete from chat_users where username='"+userName+"' and roomname='"+roomname+"'";
            connection.query(sql, function (serr1, result1) {
...
//                        console.log(sql1);
        ...
        var getGameRequestCountSocket = function(socketId, sockets){
            for(var socketKey in sockets){
                var socketObj = sockets[socketKey];
                if(socketObj.username === socketId){
                    console.log('Play Screen - getGameRequestCountSocket - socketObj.game_request_count = '+socketObj.game_request_count);// Comment this line
                    return socketObj.game_request_count;
                }
            }
            return 0;
        };// End of findSocket function
...
//                console.log(JSON.stringify(usr_json));
        ...
        function updateOtherUserStatus(othUsr,callback){
            ...
            var sql="update user_status set status='play', isPlaying=0 where user_id='"+othUsr+"'";//, isPlaying=0 added on 23 Dec 2015
//            console.log(sql);
            connection.query(sql, function (sqlerr, result) {
                if(!sqlerr){
                    callback("updated");
        ...
        function updateOtherUserStatusForDisconnection(othUsr,callback){ // Fixation other user status goen upadted if a1st user quite an app - 25 JAn 2016
            if(connection===null){
                connection=sqlConnection.handleDisconnect();
            }
            var sql="update user_status set isPlaying=0 where user_id='"+othUsr+"'";//, isPlaying=0 added on 23 Dec 2015
//            console.log(sql);
            connection.query(sql, function (sqlerr, result) {
                if(!sqlerr){
                    callback("updated");
...
//                    console.log(rows.length);
                    if(rows.length>0){
                        ...
                        var query="insert into blocked_user(blocked_user_id,user_id,isFriend) values('"+toUser+"','"+fromUser+"',1)";//  isFriend flag added on 7 Jan 2016
                        connection.query(query, function (err1, result1) {
                            if (!err1) {
                                resCallBack("Blocked");
                            }
        ...
        //Start - 8 Feb 2016 - anas 
        function setGameRequestFlag(r,fromUser,toUser,resCallBack){
            if(connection===null){
                connection=sqlConnection.handleDisconnect();
            }
                        var query="insert into blocked_user(blocked_user_id,user_id,isGameRequestRejected) values('"+toUser+"','"+fromUser+"',1)";
                        console.log('setGameRequestFlag Sql Query :'+ query);
                        connection.query(query, function (err1, result1) {
                            if (!err1) {
                                resCallBack("isGameRequestRejected flag set in db");
                            }
                            else
                            {
                              console.log('isGameRequestRejected Error: '+err1);
                            }
                        });
        }

        function deleteAllGameRequestFlag(r,mainUserFbID,resCallBack){
            if(connection===null){
                connection=sqlConnection.handleDisconnect();
            }
                        var query="Delete from blocked_user where (blocked_user_id='"+mainUserFbID+"' and isGameRequestRejected = 1 and isFriend = 0) or (user_id='"+mainUserFbID+"') and isGameRequestRejected = 1 and isFriend = 0";
                        console.log('deleteAllGameRequestFlag Sql Query :'+ query);
                        connection.query(query, function (err1, result1) {
                            if (!err1) {
                                resCallBack("deleteAllGameRequestFlag deleted successfully.");
                            }
                            else
                            {
                              console.log('deleteAllGameRequestFlag failed Error: '+err1);
                            }
                        });
        }
        //End
        ...
        function insertChatHistory(cc,fromUser,toUser,data,fullDate,fullTime,msg_status,roomName,cback){
            ...
            var message = mysql_real_escape_string(data);
            //console.log('Play ----> insertChatHistory ----- message : '+ message);
            var strSql = "insert into chat_history(from_userid,to_userid,text,date,status,roomname,room) values('" + fromUser + "','" + toUser + "','" + message + "','" + fullDate +" " +fullTime+"','"+msg_status+"','"+roomName+"','"+roomName+"1')";
            //console.log('strSql Query : '+ strSql);
            connection.query(strSql, function (err1, result1) {
                if (!err1) {
                    if (cc === 1) {
                        cback('Updated');
                  ...
                  console.log('Play---insertChatHistory ----> sqlConnection.closeConnection Error : '+ err1);
                  //sqlConnection.closeConnection(connection);// 12 Feb 2016 - never close server conenction
                }
            });
        }

        //start - 22 Dec 2015 - Convert special character string
        ...
        function insertChatLogToDB(count, fromUser, toUser, fullDate,fullTime ,data, msg_Status,strChatRoom,cb) {
            ...
            var message = mysql_real_escape_string(data);// 12 Feb 2016

            var strSql = "insert into chat_log(from_userid,to_userid,text,date,time,status,roomname) values('" + fromUser + "','" + toUser + "','" + message + "','" + fullDate +" " +fullTime+"','"+fullTime+"','"+msg_Status+"','"+strChatRoom+"')";
            connection.query(strSql, function (err1, result1) {
                if (!err1) {
                    if (count === 1) {
                        cb('Updated..');
                        console.log('New message inserted in chatlog..');
                    }
                }
                else{
                  console.log('Play---insertChatLogToDB ----> sqlConnection.closeConnection Error : '+ err1);
                  //sqlConnection.closeConnection(connection); // 12 Feb 2016 - never close server conenction
                }
            });
        }
        ...
        // 18 Dec 2015
        function playSendChat(data,userStatus,roomName,fromUser,toUser,mainUser,user,room) {
          var isToUserInPlayScreen = false;
            if(roomName)
                    {
                        var usersSocketIds = Object.keys(io.sockets.adapter.rooms[roomName]); // TODO Imp -  add undefined or null check on 'object'
                        var usersAttendingInRoom = _.map(usersSocketIds, function(socketClientId){
    //                        console.log(socketClientId);
                            return io.sockets.connected[socketClientId]
                        });
            ...
            if(isToUserInPlayScreen == false)
            {
              // 22 Jan 2016 -Change Full to first name- required in Other issues doc: DwindleNotes11816.pdf
              if(mainUserName !== null || mainUserName !== "undefined" || mainUserName !== "")
              {
                  var userFullNameArr =mainUserName.split(" ");
                  mainUserName=userFullNameArr[0];
                  console.log('Play --------------> User Firsr name : '+ mainUserName);
              }
              //
                ...
                var mainSocket = findSocket(toUserName, io.sockets.connected);
                        if (mainSocket) {
                            console.log('Play Screen - mainSocket = '+ mainSocket.username+' to_username = ' + toUserName);// comment this line
                            console.log('Play Screen -------------> message_from_play_screen ---->'+mainUserName);
                            mainSocket.emit('message_from_play_screen', data, fromUser, 'PlayScreen', mainUserName, 'playing');// TODO-Imp =  remove 'Event : message_from_play_screen           Message : ' +
                        };
            }//end - anas - 8 Oct 2015
            else if(userStatus != status_playing && userStatus != status_loggedOff)//(|| userStatus != status_play || userStatus == 'chat') Removed - Start - Get user current status - anas - 15 dec 2015
            {
              // 22 Jan 2016 -Change Full to first name- required in Other issues doc: DwindleNotes11816.pdf
              if(mainUserName !== null || mainUserName !== "undefined" || mainUserName !== "")
              {
              ...
              //
                ...
                var mainSocket = findSocket(toUserName, io.sockets.connected);
                        if (mainSocket) {
                          console.log('Play Screen - mainSocket = '+ mainSocket.username+' to_username = ' + toUserName);// comment this line
                            console.log('Play Screen - within main socket check -- it means user socket exist.');
                            mainSocket.emit('message_from_play_screen', data, fromUser, 'PlayScreen', mainUserName, 'playing');// TODO-Imp =  remove 'Event : message_from_play_screen           Message : ' +
                        };
            }//End
            ...
            var secondChatUser = "";
            console.log('Play ---> room.userCount '+room.userCount);// 16 Feb 2016
            if (room.userCount > 2) {
                if (connection == null) {
                    connection = sqlConnection.handleDisconnect();
                }
                console.log('Play ---> room.dwindleCount '+room.dwindleCount);// 16 Feb 2016
...
//                                console.log(cRes); 
                                ...
                                // Fixation issue when restart game play by both users --  added on 6 Jan 2015
                                 blockUsers(1,toUser,fromUser,function(cRes){
//                                console.log(cRes); 
                                });

                                 // resetGame(); // 3 jan 2016 - empty play room completely
                            });
...
//                }
    ...
    // Update isPlaying falg - 19 Dec 2015
    function updateIsPlayingFlag(user_fb_id, flag){
            console.log('Event change isPlaying status called with user_fb_id = '+ user_fb_id +' and flag = '+flag);
            if(connection===null){
                        connection = sqlConnection.handleDisconnect();
                }
                var sql = "update user_status set isPlaying = '" +flag+ "' where user_id='" + user_fb_id + "'";
                connection.query(sql, function (sqlerr, result) {
                    if (sqlerr) {
                        console.log('event_change_isPlaying_status : user not found');
                    }
                });
              }
    ...
    // Check and get user current status - anas  - 15 Dec 2015
    function getUserCurrentStatus(user_fb_id,resCallBack)
    {
      var userStatus;
            if(connection===null){
                    connection = sqlConnection.handleDisconnect();
                } 
            var query = "select status from user_status where user_id='"+user_fb_id+"'";
                    connection.query(query,function(err,rows,fields){
                       if(!err){
                           for (var i in rows) {
                               userStatus=rows[i].status;
                               console.log('current User status = '+ userStatus);
                               resCallBack(userStatus);
                               break;
                           }
                       }
                       else{
                            console.log('Error in fetching user current status'); //comment this line
                            resCallBack('Error');
                       }
                     });
    }
